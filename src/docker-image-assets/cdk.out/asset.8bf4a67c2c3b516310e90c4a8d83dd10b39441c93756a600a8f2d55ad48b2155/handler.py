...
AWS_ECR_PUBLIC_REPOSITORY_GROUP = os.getenv("AWS_ECR_PUBLIC_REPOSITORY_GROUP", "ecr")
...
def get_latest_image(client, repo_name):
    ...
    for img in images:
        if "latest" in (img.get("imageTags") or []):
            return (
                "latest",
                round(img.get("imageSizeInBytes", 0) / (1024**2), 2),
                img.get("imageDigest", "<none>"),
                singapore_time(img.get("imagePushedAt")),
            )
